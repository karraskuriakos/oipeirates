...
<div class="wptouch-icon-arrow-left back-button tappable"><!-- css-button --></div>
	...
	<div class="menu-bumper">
		<?php if ( wptouch_has_menu( 'site_menu' ) ) { ?>
			<a href="javascript:return false;" class="toggle-button slide-toggle" data-effect-target="menu">
		...
		<?php if ( function_exists( 'wptouch_fdn_show_login' ) ) { ?>
			<?php if ( wptouch_fdn_show_login() ) { ?>
				<?php if ( !is_user_logged_in() ) { ?>
					<a href="<?php echo wp_login_url( esc_url_raw( $_SERVER['REQUEST_URI'] ) ); ?>" class="login-button login-toggle no-ajax" title="<?php _e( 'Login', 'wptouch-pro' ); ?>"></a>
				<?php } else { ?>
